...
import com.a1631770.ikhwanov.myunittesting.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity(), View.OnClickListener {
	...
	override fun onClick(v: View?) {
		...
		val heigth = binding.edtHeight.text.toString().trim()
		...
		when {
			...
			TextUtils.isEmpty(heigth) -> {
				...
				val valueHeigth = heigth.toDouble()

				when(v?.id) {
					R.id.btn_save -> {
						mainViewModel.save(valueLength, valueWidth, valueHeigth)
						visible()
					}
